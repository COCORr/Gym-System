...
public class CardUtil {
	public Map<String, VipCard> cards = new HashMap<String, VipCard>(); // 所有手机卡的列表
	Map<String, List<ConsumInfo>> consumInfos = new HashMap<String, List<ConsumInfo>>(); // 所有手机卡消费记录的列表
	List<Scene> scenes = new ArrayList<Scene>();
	
	private int testSwimTime;
	public int getTestSwimTime() {
		return testSwimTime;
	}

	public void setTestSwimTime(int testSwimTime) {
		this.testSwimTime = testSwimTime;
	}

	public int getTestWaterNum() {
		return testWaterNum;
	}

	public void setTestWaterNum(int testWaterNum) {
		this.testWaterNum = testWaterNum;
	}

	private int testWaterNum;
	 ...
	 * @return 指定卡套餐余量信息
	 */
	public String showRemainDetail(String searchNumber) {
		VipCard card; // 要查询的卡
		int remainSwimTime=0;
		int remainwaterNum=0;
		int remainBallTime;
		StringBuffer meg = new StringBuffer();
			...
			if (pack instanceof SwimPackage) {
				...
				// 为了验证逻辑正确性，增加下面代码
				this.setTestSwimTime(remainSwimTime);
				this.setTestWaterNum(remainwaterNum);
